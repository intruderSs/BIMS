...
export class ViewComponent implements OnInit {
  ...
  constructor(private doctorService: DoctorService) {
    this.progressMsg = "Loading Staff Accounts From Blockchain";
  ...
  onDeleteDoctor(docId: any, docAdrr: any) {
    ...
    this.showWarn = true;
    this.progressTxt =
      "Are you sure you want to delete the Staff from the Network.";
  ...
  delete() {
    this.showWarn = false;
    this.progressTxt = "Deleting Staff....";
    this.doctorService.deleteDoctor(this.docId,this.docAdrr).then((data: any) => {
      if (data) {
        this.progressTxt = "Staff Deleted from Network";
