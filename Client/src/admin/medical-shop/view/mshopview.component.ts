...
export class MshopViewComponent implements OnInit {
  ...
  constructor(private mshopService: MshopService) {
    this.progressMsg = "Loading Validator From Blockchain";
  ...
  onDeleteMshop(shopId: any, shopAdrr: any) {
    ...
    this.showWarn = true;
    this.progressTxt =
      "Are you sure you want to delete the Validator from the Network.";
  ...
  delete() {
    this.showWarn = false;
    this.progressTxt = "Deleting Validator....";
    this.mshopService.deleteMshop(this.shopId, this.shopAdrr).then((data: any) => {
      if (data) {
        this.progressTxt = "Validator Deleted from Network";
